import React from "react";
import "./lookbook.scss"
export default function LookBook(props) {
    return(
        <>
           
  <div className="lookbook">
    <section className="banner">
      <h1>spring 2020 lookbook</h1>
    </section>
  </div>
  <div className="lookbook">
    <section className="sr_row container-fluid">
      <div className="row album_book">
        <div className="col-md-6 col-lg-6 col-xl-6">
          <div className="book">
            <div className="book_detail">
              <div className="bg_img">
                <img src="/img/img_lookbooks/model1.webp" alt=" model"/>
                <div className="outfit">
                  <strong><a href="/">Coverall</a></strong>
                  <strong><a href="/">Mini Quick Pack</a></strong>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="col-md-6 col-lg-6 col-xl-6">
          <div className="container-fluid">
            <div className="row">
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img bg_img--notFull">
                      <img alt = " model"src="/img/img_lookbooks/model2.webp" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img">
                      <img alt = " model"src="/img/img_lookbooks/model3.webp" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="row">
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img">
                      <img alt = " model"src="/img/img_lookbooks/model4.webp" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img bg_img--notFull">
                      <img alt = " model"src="/img/img_lookbooks/model5.webp" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section className="sr_row container-fluid">
      <div className="row album_book">
        <div className="col-md-6 col-lg-6 col-xl-6">
          <div className="book">
            <div className="book_detail">
              <div className="bg_img">
                <img alt = " model"src="/img/img_lookbooks/model6.webp" />
                <div className="outfit">
                  <strong><a href="/">Coverall</a></strong>
                  <strong><a href="/">Mini Quick Pack</a></strong>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="col-md-6 col-lg-6 col-xl-6">
          <div className="container-fluid">
            <div className="row">
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img bg_img--notFull">
                      <img alt = " model"src="/img/img_lookbooks/model1.webp" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img">
                      <img alt = " model"src="/img/img_lookbooks/model2.webp" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="row">
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img">
                      <img alt = " model"src="/img/img_lookbooks/model3.webp" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img bg_img--notFull">
                      <img alt = " model"src="/img/img_lookbooks/model4.webp" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
    <section className="sr_row container-fluid">
      <div className="row album_book">
        <div className="col-md-6 col-lg-6 col-xl-6">
          <div className="book">
            <div className="book_detail">
              <div className="bg_img">
                <img alt = " model"src="https://i.shgcdn.com/9c130b99-70a0-4a80-91d5-e286589016d7/-/format/auto/-/preview/3000x3000/-/quality/lighter/" />
                <div className="outfit">
                  <strong><a href="/">Coverall</a></strong>
                  <strong><a href="/">Mini Quick Pack</a></strong>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="col-md-6 col-lg-6 col-xl-6">
          <div className="container-fluid">
            <div className="row">
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img bg_img--notFull">
                      <img alt = " model"src="/img/img_lookbooks/model5.webp" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img">
                      <img alt = " model"src="/img/img_lookbooks/model6.webp" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="row">
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img">
                      <img alt = " model"src="/img/img_lookbooks/model1.webp" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="col-md-6 col-lg-6 col-xl-6">
                <div className="book">
                  <div className="book_detail">
                    <div className="bg_img bg_img--notFull">
                      <img alt = " model"src="https://i.shgcdn.com/9c130b99-70a0-4a80-91d5-e286589016d7/-/format/auto/-/preview/3000x3000/-/quality/lighter/" />
                      <div className="outfit">
                        <strong><a href="/">Coverall</a></strong>
                        <strong><a href="/">Mini Quick Pack</a></strong>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>                    
        </div>
      </div>
    </section>
  </div>

        </>
    )
}
